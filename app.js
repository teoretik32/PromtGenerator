...
function init() {
  ...
  SECTIONS.forEach((sec) => {
    html += `<section class="section"><div class="section-header"><span class="section-title">${sec.title}</span><span class="section-badge">${sec.mode === 's' ? 'один' : 'несколько'}</span></div><div class="tags">`;
    sec.tags.forEach((tag, i) => {
      html += `<button class="tag" id="t_${sec.id}_${i}" title="${tag.p}" onclick="toggleTag('${sec.id}', ${i})">${tag.l}</button>`;
    });
    html += '</div></section>';
  });
